...
public class HandTypeTest
{
     ...
     * translated hands. We recreate these for each test method, in case the
     * test method damages the data.
     * 
     * @Before public void makeHands() { highCardHands = makeHands(highCards);
     *         pairHands = makeHands(pairs); twoPairHands = makeHands(twoPairs);
     *         }
     */

    /**
     * This is a test method for the isOnePair method in HandType. We expect all
     * HighCard hands to be false, all OnePair hands to be true, all TwoPair
     * hands to be true, etc.
     * 
     * @Test public void testIsOnePair() { for (ArrayList<Card> hand :
     *       highCardHands) { assertFalse(HandType.isOnePair(hand)); } for
     *       (ArrayList<Card> hand : pairHands) {
     *       assertTrue(HandType.isOnePair(hand)); } for (ArrayList<Card> hand :
     *       twoPairHands) { assertTrue(HandType.isOnePair(hand)); // Two-pair
     *       contains a pair } }
     */
     ...
     * 
     * @Test public void testIsTwoPair() { for (ArrayList<Card> hand :
     *       highCardHands) { assertFalse(HandType.isTwoPair(hand)); } for
     *       (ArrayList<Card> hand : pairHands) {
     *       assertFalse(HandType.isTwoPair(hand)); } for (ArrayList<Card> hand
     *       : twoPairHands) { assertTrue(HandType.isTwoPair(hand)); } }
     */
    ...
    /**
     * Create a card from a 2-character String. First character is the rank
     * (2-9, T, J, Q, K, A) Second character is the suit (C, D, H, S)
     * 
